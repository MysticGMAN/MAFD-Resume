      ...
      * This program will be the first time we've hadled multiple output
      *  files, and multiple programs. Please run this file first with the
      *  other cbl in the same directory. As, this cbl will run the other
      ...
      *  Formatted output file of all the 
         select salary-report-out
             assign to "../../../data/A5-SalaryReport-5A.out"
             organization is line sequential.
      *  Output file for 5B cbl
         select salary-report-dat
      ...
      * opens an ouput file as report-line
      ...
      * the size of the report-line
       ...
       fd salary-report-dat
           data record is non-grad-out
           record contains 36 characters.
      * the size of the report-line
       01 non-grad-out         pic x(36).
      *
      ...
      * the main header for each page
       01 ws-heading-title.
         ...
         05 ws-title           pic x(27) value
         "GRAD EMPLOYEE SALARY REPORT".
      ...
      * the last footer for the average increase pt 1
       01 ws-average-footer-one.
         ...
         05 filler             pic x(3) value spaces.
         05 ws-analyst-avg-str pic x(8) value "ANALYST=".
         05 filler             pic x(3) value spaces.
         05 ws-analyst-avg     pic z(3),z(3).99.
         05 filler             pic x(5) value spaces.
         05 ws-sen-avg-str     pic x(9) value "SEN PROG=".
         05 filler             pic x(1) value spaces.
         05 ws-sen-avg         pic z(3),z(3).99.
      * pt 2 of the average footer
       01 ws-average-footer-two.
         05 filler             pic x(21) value spaces.
      ...
      * 77 storage variables for calculations in our compute statments
       ...
       77 ws-sen-prog-str      pic x(8) value "SEN PROG".
       77 ws-prog-str          pic x(4) value "PROG".
       
       77 ws-analyst-per       pic 99v9 value 12.8.
       77 ws-sen-prog-per      pic 9v9 value 9.3.
       ...
       77 ws-analyst-pay-tot   pic 9(7)v99 value 0.
       77 ws-analyst-inc-tot   pic 9(7)v99 value 0.
       77 ws-sen-pay-tot       pic 9(7)v99 value 0.
       77 ws-sen-inc-tot       pic 9(7)v99 value 0.
      ...
      * reads the next line in the file and changes the flag if reaches the end
           read salary-report-in next record into salary-in
               at end
                   move 'Y' to ws-eof-flag
           end-read.

       200-who-is-who.
      * we need to figure out who is smarticle. Or who paid money to be perceived as smarter
         evaluate sr-edu-code
             when "G"
                 if (ws-line-count < 1) then
                   perform 350-write-header
                 end-if
                 if sr-years greater 15 then
                   perform 210-analyst
                 end-if
                 if sr-years greater or equal 7
                   and sr-years less or equal 15 then
                   perform 220-sen-prog
                 end-if
                 if sr-years less 7 and sr-years greater 2 then
                   perform 230-prog
                 end-if
                 if sr-years less or equal 2
                   perform 240-unclassified
                 end-if
                 perform 250-diff-math
                 perform 400-write-pages
             when "N"

                 write non-grad-out from salary-in

           end-evaluate.

       210-analyst.
      * analyst calcs and counts
           ...
           add ws-new-sal-tmp  to ws-analyst-inc-tot.
           add sr-salary       to ws-analyst-pay-tot.

       220-sen-prog.
      * senior prog calcs and counts
           move ws-sen-prog-str  to ws-position.
           move ws-sen-prog-per  to ws-increase-per.
           compute ws-pay-inc rounded =
             sr-salary * (ws-sen-prog-per / 100).
           move ws-pay-inc       to ws-increase-pay.
           compute ws-new-sal-tmp = ws-pay-inc + sr-salary.
           add 1                 to ws-sen-count.
           move ws-new-sal-tmp   to ws-new-sal.
           add ws-new-sal-tmp    to ws-sen-inc-tot.
           add sr-salary         to ws-sen-pay-tot.

       230-prog.
      * programmer calcs and counts
      ...
      * unclassified calcs and counts
       ...
       250-diff-math.
      * not really doing math unless you define math as simple addition.
      ...
      * writing the average footer to the file
         ...
         write salary-line from ws-average-footer-one
           after advancing 1 lines.
         write salary-line from ws-average-footer-two.
         write salary-line from ws-budget-diff-footer
           after advancing 2 lines.
       700-avg-math.
      * doing all the average math for the main footer
         compute ws-analyst-avg rounded =
           (ws-analyst-inc-tot - ws-analyst-pay-tot)
             / ws-analyst-tot-count.

         compute ws-sen-avg rounded =
           (ws-sen-inc-tot - ws-sen-pay-tot)
             / ws-sen-tot-count.
